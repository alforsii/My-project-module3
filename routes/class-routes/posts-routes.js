...
router.get('/:classId/posts', (req, res) => {
  Class.findById(req.params.classId)
    .populate({
      ...
      populate: [
        { path: 'authorPost' },
        { path: 'classwork', populate: [{ path: 'students' }] },
      ],
    })
    .then((foundClass) => {
      let posts;
    ...
    .catch((err) =>
      console.log(`Error while getting all classroom posts${err}`)
    );
});
  ...
  Post.create({
    ...
    .then((newPost) => {
      Class.findByIdAndUpdate(
        ...
        .catch((err) =>
          console.log(
            `Error while updating classroom posts after creating a new post ${err}`
          )
        );
    })
    .catch((err) =>
      console.log(`Error in server while creating new post ${err}`)
    );
});

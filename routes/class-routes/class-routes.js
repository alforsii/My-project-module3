...
function getAllClasses(id, req, res) {
  User.findById(id)
    ...
    .then((userFromDB) => {
      ...
      const updatedArchiveClasses = removeUsersPassword(
        userFromDB.archive,
        req
      );
      res.status(200).json({
        classes: updatedClasses,
        removedClasses: updatedArchiveClasses,
      });
    })
...
function removeUsersPassword(classes, req) {
  return classes.map((eachClass) => {
    ...
    eachClass.teachers = eachClass.teachers
      .map((teacher) => {
...
function createNewClass(userId, classBody, res) {
  ...
  if (!name) {
    res
      .status(401)
      .json({ message: 'Please provide at least the name of your class!' });
    return;
  }
  ...
  User.findByIdAndUpdate(req.user._id, { $pull: { classes: classId } })
    .then((updatedUser) => {
      User.findByIdAndUpdate(
        ...
        .catch((err) =>
          console.log(`Error while adding class to archive ${err}`)
        );
    })
    .catch((err) =>
      console.log(`Error while moving removing class to archive ${err}`)
    );
});
...
//=--=-=-=-=-=-=-=-=-=-=-=-==-=-=-=-=-=-=-==-=-=-=-=-=-=-=-=-=-=-==-=-
router.post('/update/class-image', uploadCloud.single('image'), (req, res) => {
  const { classId } = req.body;
